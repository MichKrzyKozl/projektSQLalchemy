...
def get_movies(category=None, sort=None, db: Session = Depends(get_db)):
    query = (
        db.query(Movie, func.avg(MovieRating.value).label("avg_rating"))
...
def get_movie_role_actors(movie_id: int, db: Session = Depends(get_db)):
    return db.query(Actor).join(MovieRole).filter(MovieRole.movie_id == movie_id).all()
...
def create_movie_role_review(review_data: ReviewCreate, db: Session = Depends(get_db)):
    ...
    existing = (
        db.query(MovieRoleRating)
        .filter(
            MovieRoleRating.user_id == user_id,
            MovieRoleRating.role_id == review_data.reviewed_id,
        )
        .first()
    )
